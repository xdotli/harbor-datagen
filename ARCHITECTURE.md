...
├── instruction.md         # Natural language problem description (what agents see)
├── environment/           # Buggy application environment
│   ├── Dockerfile        # Container build instructions
│   ├── requirements.txt  # Python dependencies (pinned versions)
│   ├── start.sh          # Service initialization script
│   ├── start_*.sh        # Individual service startup scripts
│   └── app/              # Application code with intentional bugs
├── solution/             # Reference implementation
│   ├── solve.sh         # Script that applies the fix
│   └── reference/       # Working code files
├── tests/                # Verification and grading
│   ├── test.sh          # Main test runner (entry point)
│   └── test_*.py        # Test cases (pytest)
└── example_runs/         # Historical agent execution data
    └── <run-id>/
        ├── agent/       # Agent trajectory and commands
        ├── verifier/    # Test output and rewards
...
- **❌ DEPRECATED**: Task is no longer maintained
...
┌──────────────────────────────────────────────────────────────────┐
│  Harbor Trial Runner                                             │
│  ├─ Load task from task.toml                                    │
│  ├─ Create Docker container from environment/Dockerfile         │
│  ├─ Start container with resource limits (CPU, memory)          │
│  ├─ Upload task files to container                              │
│  └─ Execute start.sh to initialize services                     │
└──────────────────────┬───────────────────────────────────────────┘
                       │ (3-5 seconds)
                       ▼
Step 2: Agent Setup
┌──────────────────────────────────────────────────────────────────┐
│  Agent Installation (Claude Code, OpenHands, etc.)              │
│  ├─ Install agent runtime in container                          │
│  ├─ Configure agent with model API keys                         │
...
┌──────────────────────────────────────────────────────────────────┐
│  Agent Problem-Solving Loop                                      │
│  ├─ Agent reads instruction.md                                  │
│  ├─ Agent explores codebase                                     │
│  ├─ Agent diagnoses bug                                         │
│  ├─ Agent implements fix                                        │
│  └─ Agent validates solution (optional)                         │
│                                                                  │
│  Captured Data:                                                  │
│  ├─ agent/trajectory.json (action sequence)                     │
│  ├─ agent/command-*/ (command executions)                       │
│  └─ agent/sessions/ (IDE session data)                          │
└──────────────────────┬───────────────────────────────────────────┘
                       │ (240-600 seconds)
                       ▼
Step 4: Verification
┌──────────────────────────────────────────────────────────────────┐
│  Harbor Verifier                                                 │
│  ├─ Upload tests/ directory to container → /tests               │
│  ├─ Execute: bash /tests/test.sh | tee test-stdout.txt          │
│  ├─ Capture stdout/stderr to verifier/test-stdout.txt           │
│  ├─ Download /logs/verifier/reward.txt                          │
│  └─ Parse reward: "1" or "0" → float                            │
│                                                                  │
│  Output:                                                         │
...
┌──────────────────────────────────────────────────────────────────┐
│  Trial Result Generation                                         │
│  ├─ Aggregate timing data (setup, execution, verification)      │
│  ├─ Collect agent metrics (tokens, cost, commands)              │
│  ├─ Package verifier results (reward, test output)              │
│  ├─ Capture exception info (if any failures)                    │
│  └─ Write result.json                                           │
│                                                                  │
│  Result Structure:                                               │
│  {                                                               │
│    "trial_name": "auth_token_race_condition__abc123",           │
│    "agent_info": {"name": "claude-code", ...},                  │
│    "agent_result": {"n_input_tokens": 1706720, ...},           │
...
│  ├─ Remove container (optional, based on config)                │
│  └─ Upload results to storage (if configured)                   │
└──────────────────────────────────────────────────────────────────┘
```
...
1. Request A reads refresh token (exists) ✓
2. Request B reads refresh token (exists) ✓
3. Request A validates token ✓
4. Request B validates token ✓
5. Request A deletes token
6. Request A creates new token X
7. Request B deletes token **← ERROR: Deletes Request A's new token X!**
...
- Service types (FastAPI → Express, Django, Rails, etc.)
- Test frameworks (pytest → jest, mocha, cargo test, etc.)

**Example: Adapting to JavaScript/Node.js**:
```
task-name/
├── task.toml                # Same format
├── instruction.md           # Same format
├── environment/
│   ├── Dockerfile          # FROM node:18-alpine
│   ├── package.json        # npm dependencies
│   └── app/                # JavaScript code with bugs
├── solution/
│   └── solve.sh            # npm install, copy fixes
└── tests/
    ├── test.sh             # npm test
    └── *.test.js           # Jest tests
```
...
SWE-bench
├─ Real GitHub repositories (django, sympy, requests, etc.)
├─ Historical production bugs
├─ Complex test environments
└─ Strength: Real-world realism

SWE-Lancer  
├─ Real Upwork freelance tasks (Expensify App)
├─ Playwright browser tests
├─ Production application
└─ Strength: Real freelance job quality

Harbor-datagen (This Project)
├─ Agent-generated synthetic tasks
├─ Focused, containerized environments
├─ Controlled difficulty and diversity
└─ Strength: Scalable, rapid iteration
